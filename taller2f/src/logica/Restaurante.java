	...
	private void cargarIngredientes (File archivoIngredientes) {
		...
		try  {
			...
			while ((bfRead = bf1.readLine() )!= null) {
				...
				int caloriasIngrediente = (int) Math.floor(Math.random()*200+1);
				
				Ingrediente ingrediente= new Ingrediente(nombre,Integer.valueOf(precio),caloriasIngrediente);
	...
	private void cargarMenu (File archivoMenu) {
		...
		try  {
			...
			while ((bfRead = bf1.readLine() )!= null) {
				...
				String nombre = partes[0];
				String precio = partes[1];
				int caloriasMenu = (int) Math.floor(Math.random()*200+1);
				ProductoMenu producto= new ProductoMenu(nombre,Integer.valueOf(precio),caloriasMenu);
	...
	private void cargarCombos (File archivoCombos) {
		...
		try  {
			...
			while ((bfRead = bf1.readLine() )!= null) {
				...
				int caloriasTotales = (int) Math.floor(Math.random()*200+1);
				Combo combo= new Combo(nombre,Double.parseDouble(descuento.substring(0, descuento.length()-1)),caloriasTotales);
